...
func (p *Provider) registerHandlers() {
	...
	// Importe l'historique des conversations envoyé par WhatsApp juste
	// après le pairage (voir history.go).
	p.client.AddEventHandler(p.handleHistorySync)
	...
	// Reconnexion automatique : whatsmeow ne se reconnecte JAMAIS tout
	// seul après une coupure réseau (comportement documenté de la
	// librairie) — sans ce gestionnaire, une session pourtant "liée" côté
	// WhatsApp reste morte côté serveur dès la première coupure, ce qui
	// empêchait la réception de tout message en production.
	p.client.AddEventHandler(func(evt interface{}) {
		switch evt.(type) {
		case *events.Disconnected:
			log.Println("whatsmeow: connexion perdue, tentative de reconnexion...")
			if err := p.client.Connect(); err != nil {
				log.Println("whatsmeow: échec de la reconnexion automatique:", err)
			} else {
				p.setStatus(whatsapp.StatusConnected)
			}
		case *events.LoggedOut:
			// Déconnexion volontaire depuis le téléphone (Appareils liés →
			// délier) : pas de reconnexion automatique, il faut rescanner.
			log.Println("whatsmeow: session déliée depuis le téléphone")
...
// timestampFromUnix convertit un timestamp Unix (secondes) tel que renvoyé
// par les messages d'historique WhatsApp en time.Time.
func timestampFromUnix(sec uint64) time.Time {
