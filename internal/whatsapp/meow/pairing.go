...
func (p *Provider) RequestPairingCode(ctx context.Context, phoneNumber string) (string, error) {
	...
	// Protection importante : si une connexion QR est en cours (ou déjà
	// réussie), on ne touche à rien. Recréer le client pendant qu'une
	// connexion QR est active/vient de réussir a déjà causé des
	// déconnexions inattendues en production. Le code d'appairage n'est
	// autorisé que sur une session encore jamais tentée.
	status := p.Status()
	if status != whatsapp.StatusDisconnected {
		return "", fmt.Errorf("une connexion est déjà en cours ou établie (statut: %s) — déconnecte-toi d'abord si tu veux utiliser le code d'appairage", status)
	}
